...
struct Texts {
    ...
    combined_title: &'static str,
    combined_body: &'static str,
...
fn texts(locale: Locale) -> Texts {
    match locale {
        Locale::English => Texts { name: "Health Reminder", already_running: "Another instance of Health Reminder is already running.", drink_title: "🥤 Time to drink water", drink_body: "Take a short break and drink a glass of water.", exercise_title: "🙆‍♂️ Time to move", exercise_body: "Stand up and stretch for a few minutes.", combined_title: "🥤🙆‍♂️ Water and movement break", combined_body: "Drink a glass of water, then stand up and stretch for a few minutes.", test_title: "🔔 Test notification", test_body: "Health Reminder notifications are working.", paused_tooltip: "Paused", disabled_tooltip: "No reminders enabled", next: "Next", water: "water", movement: "exercise", pause: "Pause reminders", off: "Off", startup: "Start with Windows", drink: "Water interval", exercise: "Exercise interval", settings: "Settings", intervals: "Intervals", language: "Language", english: "English", spanish: "Spanish", portuguese: "Portuguese", about: "About", about_title: "About Health Reminder", version_label: "Version", about_attribution: "Developed by Rafael Tinoco using Claude", test: "Test notification", exit: "Exit", error: "Could not update Windows startup settings." },
        Locale::Spanish => Texts { name: "Recordatorio de salud", already_running: "Ya hay otra instancia de Recordatorio de salud en ejecución.", drink_title: "🥤 Hora de beber agua", drink_body: "Haz una pausa y bebe un vaso de agua.", exercise_title: "🙆‍♂️ Hora de moverse", exercise_body: "Levántate y estira durante unos minutos.", combined_title: "🥤🙆‍♂️ Pausa para hidratarse y moverse", combined_body: "Bebe un vaso de agua y luego levántate para estirar durante unos minutos.", test_title: "🔔 Notificación de prueba", test_body: "Las notificaciones funcionan correctamente.", paused_tooltip: "Pausado", disabled_tooltip: "No hay recordatorios activos", next: "Próximo", water: "agua", movement: "ejercicio", pause: "Pausar recordatorios", off: "Desactivado", startup: "Iniciar con Windows", drink: "Intervalo de agua", exercise: "Intervalo de ejercicio", settings: "Configuración", intervals: "Intervalos", language: "Idioma", english: "Inglés", spanish: "Español", portuguese: "Portugués", about: "Acerca de", about_title: "Acerca de Health Reminder", version_label: "Versión", about_attribution: "Desarrollado por Rafael Tinoco con Claude", test: "Notificación de prueba", exit: "Salir", error: "No se pudieron actualizar las opciones de inicio de Windows." },
        Locale::Portuguese => Texts { name: "Lembrete de saúde", already_running: "Já existe outra instância do Lembrete de saúde em execução.", drink_title: "🥤 Hora de beber água", drink_body: "Faça uma pausa e beba um copo de água.", exercise_title: "🙆‍♂️ Hora de se movimentar", exercise_body: "Levante-se e alongue-se por alguns minutos.", combined_title: "🥤🙆‍♂️ Pausa para beber água e se movimentar", combined_body: "Beba um copo de água e depois levante-se para se alongar por alguns minutos.", test_title: "🔔 Notificação de teste", test_body: "As notificações estão funcionando.", paused_tooltip: "Pausado", disabled_tooltip: "Nenhum lembrete ativo", next: "Próxima", water: "água", movement: "exercício", pause: "Pausar lembretes", off: "Desativado", startup: "Iniciar com o Windows", drink: "Intervalo de água", exercise: "Intervalo de exercício", settings: "Configurações", intervals: "Intervalos", language: "Idioma", english: "Inglês", spanish: "Espanhol", portuguese: "Português", about: "Sobre", about_title: "Sobre o Health Reminder", version_label: "Versão", about_attribution: "Desenvolvido por Rafael Tinoco usando Claude", test: "Notificação de teste", exit: "Sair", error: "Não foi possível atualizar as configurações de inicialização do Windows." },
    }
}
...
fn show_menu(hwnd: Hwnd, state: &AppState) {
    ...
    unsafe {
        ...
        add_menu_item(menu, IDM_PAUSE, t.pause);
...
unsafe extern "system" fn window_proc(hwnd: Hwnd, message: u32, w_param: Wparam, l_param: Lparam) -> Lresult {
    ...
    if message == WM_TIMER && w_param == TIMER_ID && !state_ptr.is_null() {
        ...
        if !state.paused {
            let current = now();
            let drink_due = state.water_enabled && current >= state.next_drink;
            let exercise_due = state.exercise_enabled && current >= state.next_exercise;
            let t = texts(state.locale);
            match (drink_due, exercise_due) {
                (true, true) => {
                    notify(hwnd, t.combined_title, t.combined_body);
                    state.next_drink = current + Duration::from_secs(state.drink_seconds);
                    state.next_exercise = current + Duration::from_secs(state.exercise_seconds);
                }
                (true, false) => { notify(hwnd, t.drink_title, t.drink_body); state.next_drink = current + Duration::from_secs(state.drink_seconds); }
                (false, true) => { notify(hwnd, t.exercise_title, t.exercise_body); state.next_exercise = current + Duration::from_secs(state.exercise_seconds); }
                (false, false) => {}
            }
        }
...
fn main() {
    ...
    if unsafe { GetLastError() } == ERROR_ALREADY_EXISTS {
        let body = wide(t.already_running);
        let caption = wide(t.name);
        unsafe {
            MessageBoxW(null_mut(), body.as_ptr(), caption.as_ptr(), 0x40);
            CloseHandle(_instance_mutex);
        }
        return;
    }
